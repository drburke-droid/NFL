...
# all-time observed max winning bid per position (finance-calibrated): the hard ceiling for
# any displayed Exp $ - the league has literally never paid more
posmax = {pos: round(max(v for (nm, s), v in auction.items() if pos_of[(nm, s)] == pos))
          for pos in SKILL}
print("calibrated all-time positional maxima:", posmax)
...
js = ("// AUTO-GENERATED by scripts/build_price_anchor.py - per-player expected-price anchors\n"
      ...
      "// PRICE_POSMAX = the most this league has EVER paid at each position (calibrated $):\n"
      "// no displayed Exp $ may exceed it - predictions need historical precedent.\n"
      f"const PRICE_ANCHOR = {json.dumps(anchors)};\n"
      f"const PRICE_ANCHOR_W = {BEST_W};\n"
      f"const PRICE_POSMAX = {json.dumps(posmax)};\n")
