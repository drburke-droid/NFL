...
Design goal: isolate the DRAFT-DAY edge. Every team — bots and us — uses the
IDENTICAL weekly projection for start/sit and waivers (equal in-season info), and
the same FAAB logic. The only thing that differs is draft valuation: bots use real
ADP (2021-25) / prior-year production x a fixed draft style; OUR team uses
walk-forward projections + optimal VORP + value-leap/fade tilt.

14-week H2H season, top-6 playoffs (wk15-17). FAAB $100/team, weekly blind bids.
K/DST streamed (modeled as an equal flat contribution — they're near-random and
everyone streams optimally). Keepers <=3, finish inflation (champ +5/top6 +3/
7-11 +2/last +0), 3-yr cap. Report: outputs/models/LEAGUE_SIM_V2.md.
"""
...
def round_robin(teams, weeks):
    n = len(teams); arr = list(teams); sched = []
...
class Team:
    def __init__(self, tid, style):
        self.tid, self.style = tid, style; self.faab = 100
        self.keepers = {}; self.reset()
    def reset(self):
    ...
    def value(self, base, pos, orank, isr):
        ...
        v = base * L1.style_mult(self.style, pos, orank, isr)
        return v * (0.35 if slot == "bench" else 1.0)
...
def auction(teams, bot_val, our_val, pos_of, rookies, kept):
...
def main():
    ...
    for Y in YEARS:
        ...
        sched = round_robin(list(range(12)), REG_WEEKS)
        ...
        def lineup_actual(t, w):
            ...
            for pos in cand: cand[pos].sort(key=lambda x: -x[0])      # choose by PROJECTION
            used = {"QB": 0, "RB": 0, "WR": 0, "TE": 0}; s = 0.0
            for pos, n in [("QB", 1), ("RB", 2), ("WR", 2), ("TE", 1)]:
                for proj, act in cand.get(pos, [])[:n]: s += act
                used[pos] = min(n, len(cand.get(pos, [])))
            flexc = []
            for pos in FLEX: flexc += cand.get(pos, [])[used[pos]:]
            if flexc: s += max(flexc, key=lambda x: x[0])[1]          # FLEX by projection, take its actual
            return s + KDST_FLAT

        for w, pairs in enumerate(sched, start=1):
            waivers(teams, Y, w, pos_of, wproj, WACT)
            for a, b in pairs:
                ta, tb = teams[a], teams[b]; sa, sb = lineup_actual(ta, w), lineup_actual(tb, w)
                pf[a] += sa; pf[b] += sb
                if sa >= sb: wins[a] += 1
                else: wins[b] += 1

        # ---- standings + playoffs ----
        seed = sorted(range(12), key=lambda i: (-wins[i], -pf[i]))
        po = seed[:6]
        def game(i, j, w): return i if lineup_actual(teams[i], w) >= lineup_actual(teams[j], w) else j
        w1 = game(po[2], po[5], 15); w2 = game(po[3], po[4], 15)          # wildcards
        better_w, worse_w = sorted([w1, w2], key=lambda x: seed.index(x))
        semiA = game(po[0], worse_w, 16); semiB = game(po[1], better_w, 16)  # 1-seed gets lowest
        ...
        # ---- keepers ----
        seas_tot = dict(zip(skY.player_id, skY.pts))
...
def waivers(teams, Y, w, pos_of, wproj, WACT):
    if w <= 1: return
    rostered = {p for t in teams for p, _ in t.roster}
    fa = [p for p in pos_of if p not in rostered]
    # rank FAs by recent form (last 3 actual weeks, leakage-free)
    def form(p):
        wa = WACT.get((Y, p), {}); prev = [wa[x] for x in range(max(1, w - 3), w) if x in wa]
        return np.mean(prev) if prev else 0.0
    fa = sorted(fa, key=lambda p: -form(p))[:25]
    bids = []
    for t in teams:
        bench_vals = [(form(p), p) for p, pos in t.roster]
        if not bench_vals: continue
        worst_f, worst_p = min(bench_vals)
        for p in fa[:12]:
            up = form(p) - worst_f
            if up > 2.0 and t.faab > 0:
                bid = int(min(t.faab, max(1, round(up * 3))))
                bids.append((bid, -t.tid, t, p, worst_p)); break
    bids.sort(key=lambda x: (-x[0], x[1])); taken = set(); gone = set()
    for bid, _, t, p, worst_p in bids:
        if p in taken or t.faab < bid or t.tid in gone: continue
        if worst_p not in {x[0] for x in t.roster}: continue
