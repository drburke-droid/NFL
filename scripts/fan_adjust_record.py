...
A code looks like  FAN1.2026.2.<base64url json>  where the json is
    {"n": fan name, "t": submitted-at ISO, "b": bake_id, "c": optional comment, "r": arrow rule,
     "a": [[player_index, stat_index, arrows], ...]}   stat_index -1 = a Quick-picks SWIPE on the whole
                                                        player (arrows +1 boost / -1 fade), expanded here
                                                        into one row per stat under rule "s1"
...
def rows_for(S, W, j):
    ...
    def row(p, pi, stat, v, rule):
        ...
        # pct keeps its old meaning exactly for pct10 codes (10 x presses); under u1 and s1 it is the
        # same move expressed as a percent of the baseline, from the unrounded value
        pct = 10 * v if rule == "pct10" else (round(100 * (raw - base) / base, 1) if base else "")
        return {"received_at": now, "season": S, "week": W, "fan": fan, "submitted_at": sub, "bake_id": bake.get("bake_id", ""), "comment": com, "fan_id": fid, "device_id": did, "fan_team": fav, "player_index": pi,
