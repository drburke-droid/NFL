...
for t, cand, p in teams_scored:
    ...
    for (c, r), pp in order: PROBS[c["pid"]] = round(float(pp), 3)
...
# keep-probability map for the draft tool's Mock Keepers screen (keyed by the same pid
# as keepers_2026.js candidates / BYID in index.html)
_js = ("// AUTO-GENERATED by scripts/keeper_likelihood.py — modeled keep probability per rostered\n"
       "// player: logistic (savings, cost, prior PPG, games, age) fitted on the league's own\n"
       "// 2023-25 keeper decisions, base-rate calibrated to 2.65 keeps/team, slam-dunk locks\n"
       "// (young healthy discounted stars) floored at 0.97. Run AFTER predict_keepers.py.\n"
       "const KEEPER_PROB = " + json.dumps(PROBS) + ";\n")
for _d in (os.path.join(ROOT, "docs"), os.path.join(ROOT, "outputs", "draft_tool")):
    open(os.path.join(_d, "keeper_prob_2026.js"), "w", encoding="utf-8").write(_js)
_print("wrote docs/keeper_prob_2026.js (+ mirror): " + str(len(PROBS)) + " players")
